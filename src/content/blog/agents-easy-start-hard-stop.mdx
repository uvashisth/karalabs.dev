...
No planning engine. No reasoning module. No agent runtime. `messages` is the memory. Every tool result and model response accumulates there. The model reads all of it on every iteration. That accumulated context is where the reasoning happens.
